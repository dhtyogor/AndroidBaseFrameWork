...
//    }
    ...
    private boolean verify() {
        //版本忽略
        if (mShowIgnoreVersion && AppUpdateUtils.isNeedIgnore(mActivity, mUpdateApp.getVersionNumber())) {
            return true;
        }
...
//                || !mTargetPath.startsWith(preSuffix)
    ...
    private void processData(String result, @NonNull UpdateCallback callback) {
        try {
            ...
            Log.e("xxx","==code=="+return_code);
            if (return_code.equals("0000")) {
                String data = jsonObject.getString("data");
                mUpdateApp = callback.parseJson(data);
                if (mUpdateApp.isUpdate(getContext())) {
...
//            Log.e("xxx","???"+mUpdateApp.isUpdate(getContext()));
//            if (mUpdateApp.isUpdate(getContext())) {
//                callback.hasNewApp(mUpdateApp, this);
//                //假如是静默下载，可能需要判断，
//                //是否wifi,
